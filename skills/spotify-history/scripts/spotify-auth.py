...
CREDS_FILE = Path.home() / ".openclaw" / "workspace" / "credentials" / "spotify.json"
if CREDS_FILE.exists():
    try:
        _c = json.loads(CREDS_FILE.read_text())
        CLIENT_ID = CLIENT_ID or _c.get("client_id", "")
        CLIENT_SECRET = CLIENT_SECRET or _c.get("client_secret", "")
    except Exception:
        pass
REDIRECT_URI = "http://127.0.0.1:8889/callback"
SCOPES = "user-read-recently-played user-top-read user-read-playback-state user-read-currently-playing user-modify-playback-state playlist-modify-private playlist-modify-public"
...
def main():
    ...
    server = http.server.HTTPServer(("127.0.0.1", 8889), CallbackHandler)
